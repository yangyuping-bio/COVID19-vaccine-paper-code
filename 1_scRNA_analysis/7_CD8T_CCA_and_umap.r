...
# 设置路径
# 设置路径
results_path <- "/share/home/qlab/projects/project_cvv/yyp_results_3/5_Cell_type_level_3/4_Tcell_CD8T/1_CCA"
...
targetcell <- subset(targetcell, idents = c("CD8_Tcm","CD8_Tem","CD8_Tn","CD8_NELL2")) 
...
targetcell <- subset(targetcell, idents = c("CD8T")) 
...
idents_name_list <- c("integrated_snn_res.0.6","ct_Tcell","ct_level_2","ct_level_1")
pdf(file.path(results_path, "DimPlot_res.0.6.pdf"),6,5)
...
DefaultAssay(sample.integrated) <- "RNA"
Idents(sample.integrated) <- "integrated_snn_res.0.6"
...
write.csv(sample.integrated.markers, file.path(results_path, "Markers_res.0.6.csv"))
...
sample.integrated <- readRDS( "CCA_CD8T.rds")
...
DefaultAssay(sample.integrated) <- "RNA"                         
idents_name_list <- c("ct_level_3","integrated_snn_res.0.6","ct_level_2","ct_level_1")
...
saveRDS(sample.integrated, file = "CCA_CD8T.rds")
...
targetcell_D<- subset(sample.integrated, idents = c("D_T_Platelet","D_T_NK"))
results_path <- "/share/home/qlab/projects/project_cvv/yyp_results_3/5_Cell_type_level_3/0_merge_data"
saveRDS(targetcell_D, file = paste0(results_path,"/4_Tcell_CCA_CD8T_batch_Doublet.rds"))

targetcell_D@meta.data %>%
  dplyr::group_by(ct_level_3) %>%
  dplyr::summarise(num = n(), gene = mean(nFeature_RNA), count = mean(nCount_RNA), mt = mean(percent.mt)) %>%
  arrange(num)
...
# 设置路径
# 设置路径
results_path <- "/share/home/qlab/projects/project_cvv/yyp_results_3/5_Cell_type_level_3/4_Tcell_CD8T/2_Reumap"
data_save_path <- "/share/home/qlab/projects/project_cvv/yyp_results_3/5_Cell_type_level_3/4_Tcell_CD8T/1_CCA"
# setwd()指定路径，setwd("~/project/")
...
CD8T <- readRDS(file.path(data_save_path, "CCA_CD8T.rds"))
...
# ---- subset cell
Idents(CD8T) <- "ct_level_3"
CD8T<- subset(CD8T, idents = c("CD8_Tem","CD8_Temra","CD8_Tn","CD8_NELL2"))
...
sample.integrated <- RunUMAP(targetcell, umap.method = "umap-learn", reduction = "pca", dims = 1:5)
...
sample.anchors <- FindIntegrationAnchors(
  ...
  reduction = "cca", #reduction ="rpca",
  k.anchor = 5,
  k.filter = 40, #default:k.weight = 200
  dims = 1:20,
  anchor.features = features.filtered,
  scale = FALSE
)
...
sample.integrated <- IntegrateData(
  anchorset = sample.anchors,
  dims = 1:20,
  k.weight = 40, #default:k.weight = 100
  verbose = TRUE
)
...
# ---- PCA and clustering
DefaultAssay(sample.integrated) <- "integrated"
sample.integrated <- RunPCA(sample.integrated, npcs = 10, features = var.features.filtered)
sample.integrated <- RunUMAP(sample.integrated, umap.method = "umap-learn", reduction = "pca", dims = 1:5)
sample.integrated <- FindNeighbors(sample.integrated, reduction = "pca", dims = 1:5, force.recalc = T)
...
sample.integrated$ct_level_4[sample.integrated$integrated_snn_res.0.2  == 0 ] <- "CD8_Tn"
sample.integrated$ct_level_4[sample.integrated$integrated_snn_res.0.2 %in% c(4,5) ] <- "CD8_Tcm"
sample.integrated$ct_level_4[sample.integrated$integrated_snn_res.0.2  == 1 ] <- "CD8_Temra"
sample.integrated$ct_level_4[sample.integrated$integrated_snn_res.0.2 == 2 ] <- "CD8_Tem"
sample.integrated$ct_level_4[sample.integrated$integrated_snn_res.0.2 == 3 ] <- "CD8_NELL2"
...
sample.integrated<- readRDS(file.path(data_save_path, "4_Tcell_CD8T/2_Reumap/Trajectory_CD8T.rds"))
...
head(data)
ncols <- c("#A01319", "#68A180", "#53A85F", "#C5DEBA")
pdf(file.path(results_path, "slingshot_CD8T_line2.pdf"), width = 7.5, height = 5)
...
ncols <- c("#53A85F","#99AF09","#68A180","#D8E7A9")
pdf(file.path(results_path, "slingshot_CD8T_line2.pdf"), width = 7, height = 5)
DimPlot(object = targetcell_use, reduction = "umap", label = F,
...
pdf("slingshot_Pseudotime.pdf", width =4.5, height = 3.8)
...
#saveRDS(targetcell_use, file = "Trajectory_CD8T.rds")

meta_data.df <- targetcell_use@meta.data

ncols <- c("#53A85F","#99AF09","#68A180","#D8E7A9")
pdf("1_Line_CD8T_ct_level_4.pdf", 6.3, 10.5)
#ct_level_4
