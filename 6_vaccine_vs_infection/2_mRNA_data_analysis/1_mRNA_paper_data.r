...
plot_clonotype_umap(CD4T, "figures/4_CD4T_clonotype_umap.pdf", 
                    nclos_clonotype,5,3.5,0.0001)

pdf("figures/4_CD4T_clonotype_umap_split.pdf", 18, 4)
split_conditions <- c("Clonotype_num_ld", "timepoint", "celltype.l2", "donor_id", "experiment")
...
plot_clonotype_umap(CD8T, "figures/4_CD8T_clonotype_umap.pdf", 
                    nclos_clonotype,5,3.5,0.0001)

pdf("figures/4_CD8T_clonotype_umap_split.pdf", 18, 4)
split_conditions <- c("Clonotype_num_ld", "timepoint", "celltype.l2", "donor_id", "experiment")
...
nclos_clonotype <-  c("lightgrey", "darkgrey", 
                      "#F3B1A0",  "#B22C2CFF","#573333FF")
ct_seq <- unique(mRNA_obj$celltype.l2)
clonotype_num <- as.data.frame(table(mRNA_obj$TCRab_TRB.CDR3.1))
mRNA_obj$Clonotype_num <- clonotype_num$Freq[match(mRNA_obj$TCRab_TRB.CDR3.1, clonotype_num$Var1)]
mRNA_obj <- process_clonotype_data(mRNA_obj,ct_seq)
plot_clonotype_umap(mRNA_obj, "figures/5_mRNA_obj_clonotype_umap.pdf", nclos_clonotype,8,5.5,0.0001)

pdf("figures/5_mRNA_obj_clonotype_umap_split.pdf", 28, 4)
split_conditions <- c("Clonotype_num_ld", "timepoint", 
                      "celltype.l2", "donor_id", "experiment")
lapply(split_conditions, function(split_by) plot_dim(mRNA_obj, split_by))
dev.off()
...
pdf("figures/6_CD4T_Bar_Clonotype_Levels_all.pdf",3.6,2.5)
...
# 绘制堆叠条形图
pdf("figures/6_CD4T_Bar_Clonotype_Levels.pdf",3.6,2.5)
ggplot(data, aes(x = celltype.l2, y = Count, fill = Clonotype_num_ld)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_fill_manual(values = color_mapping) +
...
pdf("figures/6_CD8T_Bar_Clonotype_Levels_all.pdf",3.6,2.5)
ggplot(data, aes(x = celltype.l2, y = Count, fill = Clonotype_num_ld)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_fill_manual(values = color_mapping) +
  labs(
...
# 载入 ggplot2 包
library(ggplot2)
CD8T_f <- CD8T@meta.data%>% filter(Clonotype_num_ld %in% c("1 < n <= 3", "3 < n <= 5",  "n > 5"))
...
# 绘制堆叠条形图
pdf("figures/6_CD8T_Bar_Clonotype_Levels.pdf",3.6,2.5)
ggplot(data, aes(x = celltype.l2, y = Count, fill = Clonotype_num_ld)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_fill_manual(values = color_mapping) +
  labs(title = "Clonotype Number", 
...
mRNA_obj <- read.csv('mRNA_obj_meta_data.csv')
