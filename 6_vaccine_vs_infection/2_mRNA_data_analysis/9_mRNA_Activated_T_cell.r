...
# 绘制分组占比条形图
pdf("figures/3_Combined_Predicted_Label_Percentage_Plot.pdf",  8.5, 3.2)
...
# 定义绘制饼图的函数
plot_pie_chart <- function(data, group_name, output_file) {
  # 筛选对应分组的数据
  ...
  pdf(output_file, width = 6, height = 3)
      # 绘制饼图
     print( ggplot(group_data, aes(x = "", y = Percentage, fill = Predicted_Label)) +
        geom_bar(width = 0.9, stat = "identity", alpha = 0.95, color = "white") +  # 设置透明度和边框颜色
        coord_polar(theta = "y") +  # 使用极坐标绘制饼图
        scale_fill_manual(values = colors) +  # 设置颜色映射
        theme_void() +  # 移除背景元素
        labs(fill = "Predicted Label", title = paste(group_name)) +  # 添加标签
           #,"\n", "Predicted Labels Distribution"
        theme(
          legend.key.size = unit(0.35, "cm"),  # 图例键大小
          legend.text = element_text(size = 14),  # 图例文本大小
          legend.title = element_text(size = 16, face = "bold"),  # 图例标题样式
          legend.spacing.x = unit(0.5, "cm"),  # 图例项间距
          legend.position = 'right',  # 图例位置
          plot.margin = unit(c(2, 2, 2, 2), "char"),  # 图的外边距
          plot.title = element_text(size = 16, hjust = 0, vjust = 0, face = "bold")  # 标题样式
        ) +
...
# 分别绘制三个分组的饼图，颜色映射保持一致
plot_pie_chart(combined_data,"Inactivated Vaccine (55918 T cells)", "figures/3_cvv_Predicted_Labels_Pie_Chart.pdf")
plot_pie_chart(combined_data, "mRNA Vaccine (61083 T cells)",  "figures/3_mRNA_Predicted_Labels_Pie_Chart.pdf")
plot_pie_chart(combined_data, "Sustained infection (49532 T cells)", "figures/3_Sustained_Predicted_Labels_Pie_Chart.pdf")
plot_pie_chart(combined_data, "Transient infection (50598 T cells)", "figures/3_Transient_Predicted_Labels_Pie_Chart.pdf")
plot_pie_chart(combined_data, "Abortive infection (30773 T cells)", "figures/3_Abortive_Predicted_Labels_Pie_Chart.pdf")
